...
#
print(len(model.raw_vocab)) # check the length of the vocabulary which was formed by Word2Vec

#The rest implements passing TOEFL tests
i = 0 #counter for TOEFL tests
number_of_tests = 80
text_file = open(base_path+'new_toefl.txt', 'r')
...
while i < number_of_tests:
            ...
            try:
                ...
                if words[0] in model_words: # check if there embedding for the query word
                    k = 1 #counter for loop iterating over 5 words in the test
                    vectors.append(model_words[words[0]])
                    while k < 5:
                        if words[k] in model_words: # if alternative has the embedding
                            vectors.append(model_words[words[k]]) #assing the learned vector
                        else: 
